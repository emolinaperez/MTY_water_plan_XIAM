...
dynamic.optimizer<-function(dir.model,dir.output,Demand.Scenarios.Table,Climate.Scenarios.Table,GW.Scenarios.Table,Desalt.Scenarios.Table,Projects_Chars,Future.ID)
{
...
#Domains
  ...
  x.fin.0<-c(as.integer(1.0), #x1
             as.integer(1.0), #x2
             as.integer(0.0), #x3 #Desalt cannot be implemented in the first period
             as.integer(1.0), #x4
             as.integer(1.0), #x5
             as.integer(1.0), #x6
             as.integer(1.0), #x7
...
#Load Futures Table
 Futures.Table<-read.csv(paste(dir.exp,"FuturesTable_27_12_2017.csv",sep="")) #UPDATE
#For testing
 Futures.Table<-subset(Futures.Table,Futures.Table$Block=="Block1") #REMOVE
...
#Load Groundwater Scenarios Table
 GW.Scenarios.Table<-read.csv(paste(dir.scenarios,"GWScenarios_27_12_2017.csv",sep="")) #UPDATE
#Load Desalt Scenarios Table
  Desalt.Scenarios.Table<-read.csv(paste(dir.scenarios,"DesaltScenarios_27_12_2017.csv",sep="")) #UPDATE
...
#
 dir.portfolios<-"C:\\Users\\L03054557\\OneDrive - Instituto Tecnologico y de Estudios Superiores de Monterrey\\Edmundo-ITESM\\Proyectos\\ProyectoAguaMty\\Phase2MTY\\ExperimentsRawOutput\\DynamicOptimalPortfolios\\Experiment_14_01_2018_Step_1\\"
...
#write result
  ...
  write.csv(OptimalPortfolios,paste(dir.output,"OptimalPortfolios_14_01_2018.csv",sep=""),row.names=FALSE)
