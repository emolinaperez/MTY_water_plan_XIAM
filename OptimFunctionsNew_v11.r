...
#Input GW Scenario in Riverware
    GW.Scenario.Name<-as.character(Futures.Table[Future.ID,"ClimateScenario"])
    GW.Scenario<-subset(GW.Scenarios.Table,GW.Scenarios.Table$ClimateScenario==GW.Scenario.Name)
...
#
 dir.portfolios<-"C:\\Users\\L03054557\\Edmundo-ITESM\\Proyectos\\ProjectoAguaMty\\Phase2MTY\\ExperimentsRawOutput\\DynamicOptimalPortfolios\\CloudLargeExperiment\\"
...
#Load Climate Scenarios Chars
  ClimateCharsPeriods<-read.csv(paste(dir.output,"ClimateCharsPeriods.csv",sep=""))
...
#Load Demand Scenarios Chars
  DemandCharsPeriods<-read.csv(paste(dir.output,"DemandCharsPeriods.csv",sep=""))
#
 dim(Futures.Chars)
...
#
 ...
 dim(Futures.Chars)
#write demand chars
  write.csv(Futures.Chars,paste(dir.output,"FuturesChars.csv",sep=""),row.names=FALSE)
