...
mty.riverware.model<-function(Run.ID,dir.model,dir.riverware.input,Demand.Scenario,Climate.Scenario,Policy) {
...
#Write climate scenario in Riverware input file
 write.xlsx2(Climate.Scenario, file=paste(dir.riverware.input,"RiversData.xlsx",sep=""),row.names=FALSE)

#Write policy scenario in Riverware input file
...
##Load Climate Scenario Table from Exp Design
  Climate.Scenario.Name<-as.character(Exp.design[Run.ID,"ClimateScenario"])
...
#Run the model
 mty.riverware.model(Run.ID,dir.model,dir.riverware.input,Demand.Scenario,Climate.Scenario,Policy)
}
...
#This is inneficient programming, revise and edit later when time allows it
####################################
#add exp.design to estimate net extra Supply
 ...
 SupplyDemand.NoExpansion<-as.data.frame(subset(SupplyDemand,SupplyDemand$Policy=="NoExpansion"))
 SupplyDemand.NoExpansion<-SupplyDemand.NoExpansion[,c("Future.ID","Year","Supply.Monterrey","Policy")]
 SupplyDemand.NoExpansion$Index<-paste(SupplyDemand.NoExpansion$Future.ID,SupplyDemand.NoExpansion$Year,sep="_")
 SupplyDemand.NoExpansion$Policy<-NULL
 SupplyDemand.NoExpansion$Year<-NULL
 SupplyDemand.NoExpansion$Future.ID<-NULL
 SupplyDemand.NoExpansion$Supply.NoEx<- SupplyDemand.NoExpansion$Supply.Monterrey
 SupplyDemand.NoExpansion$Supply.Monterrey<-NULL
 SupplyDemand.NoExpansion<-data.table(SupplyDemand.NoExpansion)

#Create Index variable un Supply demand
 SupplyDemand[,Index:=paste(Future.ID,Year,sep="_")]
#elimiate useless columns
  SupplyDemand[,Policy:=NULL]
  SupplyDemand[,Future.ID:=NULL]
  SupplyDemand[,Cost:=NULL]
#now merge
  dim(SupplyDemand)
  SupplyDemand<-merge(SupplyDemand,SupplyDemand.NoExpansion,by="Index")
  dim(SupplyDemand)
#Estimate Net effect
 SupplyDemand[,Policy.Net.Effect:=Supply.Monterrey-Supply.NoEx]
 SupplyDemand[,Index:=NULL]
 SupplyDemand[,Supply.NoEx:=NULL]
############################################################
#write database
...
#Supply Demand
  SupplyDemand<-SupplyDemand [, j=list(Supply.Monterrey = mean(Supply.Monterrey),
                                       ...
                                       Reliability = mean(Reliability),
                                       Policy.Net.Effect=mean(Policy.Net.Effect)),
                                       by= list (Run.ID)]
