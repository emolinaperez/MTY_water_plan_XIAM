...
if (is.na(Reliability.Annual.Mean)==TRUE)
{
  ...
  return(999999999)
} else if (Reliability.Annual.Mean>=0.96)
{ return(as.numeric(Portfolio.Cost))
          } else  { return(999999999) }

}

iteration.optim<-function(dir.model,Projects_Chars,Domains,ini.yr,end.yr)
{
...
if (sum(Domains[,1])==0 & portfolio.reliability(rep(0,nrow(Projects_Chars)))>=0.943)
{
 optim.portfolio<-list(par=rep(0,nrow(Projects_Chars)),value=0)
} else if (portfolio.reliability(Domains[,1])>=0.943)
{
optim.portfolio<-list(par=Domains[,1],value=portfolio.eval(Domains[,1]))
} else if (portfolio.reliability(rep(1,nrow(Projects_Chars)))>0.943)
{
...
#Domains
  ...
  portfolio1<-iteration.optim(dir.model,Projects_Chars,Domains,2015,2026)
...
#Solve for second step
  portfolio2<-iteration.optim(dir.model,Projects_Chars,Domains,2027,2038)
...
#Solve for third step
  portfolio3<-iteration.optim(dir.model,Projects_Chars,Domains,2039,2050)
