...
dynamic.optimizer2<-function(dir.model,dir.output,Demand.Scenarios.Table,Climate.Scenarios.Table,GW.Scenarios.Table,Desalt.Scenarios.Table,Projects_Chars,Future.ID,step1.options)
{
...
#  Domains<<-matrix(c(x.ini.0,x.fin.0),ncol=2)
  ...
  step1.cost<-as.numeric(
                matrix(step1.options,nrow=1,ncol=length(unique(Projects_Chars$ID_Project)))%*%matrix(Projects_Chars$Total.Investment.Millions,nrow=length(unique(Projects_Chars$ID_Project)),ncol=1)
                        )
...
#projects to be carried out in first step 1
#load file
 #step1.options<-read.csv(paste(dir.exp,"step1_options_09_01_2018.csv",sep=""))
# step1.options<-read.csv(paste(dir.exp,"step1_option_2_10_03_2018.csv",sep=""))
# step1.options<-read.csv(paste(dir.exp,"step1_option_7_10_03_2018.csv",sep=""))
# step1.options<-read.csv(paste(dir.exp,"step1_option_4_10_03_2018.csv",sep=""))
# step1.options<-read.csv(paste(dir.exp,"step1_option_8_10_03_2018.csv",sep=""))
 #step1.options<-read.csv(paste(dir.exp,"step1_option_5_10_03_2018.csv",sep=""))
 #step1.options<-read.csv(paste(dir.exp,"step1_option_1_10_03_2018.csv",sep=""))
# step1.options<-read.csv(paste(dir.exp,"step1_option_3_10_03_2018.csv",sep=""))
# step1.options<-read.csv(paste(dir.exp,"step1_option_9_10_03_2018.csv",sep=""))
step1.options<-read.csv(paste(dir.exp,"step1_option_6_10_03_2018.csv",sep=""))

#Split the experimento into #n number of computers
 sequence.of.runs<-1:length(unique(Futures.Table$Future.ID))
# Total.machines<-10
 Total.machines<-3
...
#print for view
 length(unique(Futures.Table$Future.ID))
  first
  final
for (i in unique(Futures.Table$Future.ID)[first:final])
   {
   dynamic.optimizer2(dir.model,dir.output,Demand.Scenarios.Table,Climate.Scenarios.Table,GW.Scenarios.Table,Desalt.Scenarios.Table,Projects_Chars,i,as.numeric(step1.options))
 }

#machine 81-234, 30 runs missing  c(172:194)
#machine 110-108, 30 runs missing  c(224:259)
#machhine 253-191, 30 runs missing c(278:324)
#machine 243-30, 30 runs missing  c(349:388)

#re distribute jobs

pending.runs<-c(172:194,224:259,278:324,349:388)
pending.runs<-c(896:907,958:972)

sequence.of.runs<-1:length(pending.runs)
Total.machines<-10
runs.per.machine<-length(sequence.of.runs)/Total.machines
runs<-split(sequence.of.runs, ceiling(sequence.of.runs/runs.per.machine))
...
length(pending.runs)
 first
 final
for (i in pending.runs[first:final])
for (i in pending.runs[1:27])
  {
  dynamic.optimizer2(dir.model,dir.output,Demand.Scenarios.Table,Climate.Scenarios.Table,GW.Scenarios.Table,Desalt.Scenarios.Table,Projects_Chars,i,as.numeric(step1.options))
}
 ...
 dir.portfolios<-"C:\\Users\\L03054557\\OneDrive - Instituto Tecnologico y de Estudios Superiores de Monterrey\\Edmundo-ITESM\\Proyectos\\ProyectoAguaMty\\Phase2MTY\\ExperimentsRawOutput\\DynamicOptimalPortfolios\\Experiment_06_03_2018_Step_2\\Option2\\"
...
#write result
  ...
  write.csv(OptimalPortfolios,paste(dir.output,"OptimalPortfolios_06_03_2018_Step2_Option2.csv",sep=""),row.names=FALSE)
